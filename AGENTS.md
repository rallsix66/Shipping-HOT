...
- The current working branch is `main` at `4f29a9549deaa7614dca306838a9e110d058667e` (`fix: count only current calendar sync records`), tracking `origin/main`. The approved Phase 1 calendar integrity/count repair is committed; the 2026-09-08 remaining-scope review and two Bugbot follow-up fixes are intentionally uncommitted and locally verified, with no real Provider re-verification. The retained `.tmp/ShippingHot-WebDiagnosis-20260908/` diagnostic copy remains untracked by user instruction. The historical P7 acceptance branch is `codex/shipping-hot-v3-real-data` at seal commit `ed2c8448699971328b23247508a7b91fb537ab6b`. Keep this distinction: Phase 1 work on `main` must not rewrite the historical P7 acceptance record or describe the seal as full capability coverage.
