...
- S7 `Clean Local Integrated Acceptance` is `PASS` (2026-09-14) on an isolated `.tmp/s7-local` database against the production build: fresh init + schema v13 migration + restart persistence + system-Chrome/CDP Flow A/B/C, `137 checks / 0 FAIL` (Flow A 35 / B 22 / C 26) across 5 consecutive runs, zero Provider/usage/Runtime/cache deltas, zero external egress, zero Mock leakage and untouched retained databases. It found and fixed two real defects: the `$id` detail routes were nested under their list routes (no `<Outlet/>`, so `/vessels/$id`, `/ports/$id`, `/voyages/$id` only ever rendered the list page — fixed by the `_`-suffix route naming) and an unconfigured Real-Mode Vessel Search threw a bare `Error` that surfaced as HTTP 500 instead of a coded `provider_unavailable` 503. Re-run `pnpm seed:s7-local` + `pnpm test:e2e:s7` only if later code changes touch the flows it covers. Two facts stay recorded without being "fixed": `/calendar` mounts only the bundled annual reference calendar because the operational-cache entry was deliberately removed on 2026-09-09 (the legacy `CalendarPage` stays exported but unmounted — do not "restore" it without a product decision), and the S5 freeze still holds.
