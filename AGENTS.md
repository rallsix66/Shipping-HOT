...
- The current committed baseline is `main` at `ce904192784ba6fe010c8f0afdcf531836c41b04` (`feat: add southeast asia annual reference calendar`), synchronized with `origin/main` when that commit was pushed and verified. The Thailand expansion to 23 reference items and its display adaptation passed independent review and remain uncommitted pending this submission; they do not establish nationwide completeness because the SOC annual body is still pending, and no human double-review is claimed. Retain the untracked diagnostic artifacts and the existing `pages.tsx` line-ending-only worktree state. Historical P7 remains `codex/shipping-hot-v3-real-data@ed2c8448699971328b23247508a7b91fb537ab6b`; never rewrite that seal as full capability coverage.
