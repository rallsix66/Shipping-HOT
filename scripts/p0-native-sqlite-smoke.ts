...
import { p3bVoyageEtaMigration } from "../server/database/migrations/008-p3b-voyage-eta"
...
async function openDatabase(path: string): Promise<InstanceType<typeof NativeDatabase>> {
  ...
  const migrations = [p0FoundationMigration, watchlistIsolationMigration, p1aPortDirectoryMigration, p1bMockIsolationMigration, p2aSearchFoundationMigration, p2cRuntimeFoundationMigration, p3aAisTrackingMigration, p3bVoyageEtaMigration]
    ...
    try {
      await migration.up({
        ...
        prepare: (sql: string) => ({
          ...
          get: async (...params: (string | number | boolean | null | undefined)[]) => native.prepare(sql).get(...params),
  ...
  native.prepare(`
    ...
    VALUES ('default', 8, ?, COALESCE(?, lower(hex(randomblob(16)))), ?, 'real')
...
async function reader(path: string) {
  ...
  for (const table of ["schema_migrations", "app_metadata", "port_directory_status", "vessel_watchlist", "port_watchlist", "translation_cache", "provider_usage", "provider_runtime", "sync_runs", "vessel_metadata", "vessel_search_cache", "ais_positions", "ais_latest_positions", "voyage_eta_history"]) {
