...
| Voyage / ETA | `mock` provider in default runtime; controlled Real Mode keeps the Job disabled because no real adapter/key is configured | VesselAPI ETA and optional Port Events, subject to account entitlement | No real ETA adapter; Mock adapter only | Yes: `VESSELAPI_API_KEY` | No (`credential_missing`) | No probe; ETA entitlement **not verified** | Endpoint entitlement and plan/quota unknown | `credential_missing` | Gap is recorded in `docs/voyage-provider-gap.md`; `entitlement_missing` is not asserted without an explicit Provider response; no ETA/Port Events implementation or origin guess was added. |
