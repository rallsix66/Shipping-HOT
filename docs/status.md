...
> Evidence scope: local code / configuration / Git metadata; V2.0 is sealed, V2.1 is implemented, and V2.2/V2.3/V2.4 local closeout gates passed. The corrected 2026-08-18 public re-probe verified Portcast 8/8 page/parser responses (7 fresh, 1 stale), Open-Meteo 8/8 port responses and Shekou `/ywgg/` parsing; Loadstar returned 10 items and Maritime Executive remained failed. AISStream and Calendarific remain `pending_credentials`; official-alert verification remains `live_pending`. Full lint retains four pre-existing errors and native SQLite runtime is pending on this Node 24 environment.
...
## Remaining Real Provider Verification — 2026-08-18

This pass checked the remaining credential-gated and public-source paths directly. No secret value was printed, stored or written to documentation. AISStream and Calendarific were not called because their required credentials are missing.
...
| Calendarific | `calendarific` | `CALENDARIFIC_API_KEY: missing`; no external request; five-country result has zero events and `unknown` coverage for each country | `pending_credentials` | Keep requested real mode; do not seed Mock Calendar events |
...
| TMD | `https://www.tmd.go.th/en/api/xml/CAP` | HTTP 200 `text/xml`; payload root is RSS; after the minimal registry correction from `cap` to `rss`, parser returned 7 items with issued time, channel update time and Laem Chabang relation; the RSS index does not provide verified expiry fields | `live_pending` | Keep disabled until full warning-field and lifecycle criteria are verified |
| BMKG | `https://www.bmkg.go.id/alerts/nowcast/en` | HTTP 200 `application/xml`; RSS parser returned 2 items with issued time, channel update time, severity and Jakarta relation; no verified expiry field in the index response | `live_pending` | Keep disabled until full warning-field and lifecycle criteria are verified |
| Maritime Executive | `https://maritime-executive.com/rss` | DNS resolved A/AAAA records, but TCP 443 timed out and direct HTTPS fetch failed; no RSS/XML response was received | `failed_live` | Retain the source and failure isolation; do not fabricate Feed/HOT data and do not affect Loadstar or Shekou |

The no-network all-real mode smoke requested `vessel=aisstream`, `port=portcast`, `weather=open-meteo`, `weatherAlerts=public`, `feed=public`, `calendar=calendarific`, with `schedule=mock`. Active source IDs contained `aisstream`, `calendarific`, `portcast-public`, `open-meteo-marine`, `the-loadstar`, `maritime-executive`, `shekou-official` and `mock-schedule`; `mock-vessel`, `mock-port`, `mock-weather`, `mock-port-notice` and `mock-calendar` were absent. No official alert source was active in `public` mode because JMA/TMD/BMKG remain unverified.
