...
> Evidence scope: local code / configuration / Git metadata; V2.0 is sealed, V2.1 is implemented, and V2.2/V2.3/V2.4 local closeout gates passed. The last public probe (2026-08-17) observed Portcast 8/8 page responses, Open-Meteo 8/8 port responses and mixed Feed results; this pass corrected the Portcast HCM path/source-age semantics, Open-Meteo timestamp roles and Shekou `/ywgg/` parser, but a new public re-probe is pending because the current runtime safety gate rejected external live access. AISStream and Calendarific remain `pending_credentials`. Full lint retains four pre-existing errors and native SQLite runtime is pending on this Node 24 environment.
> Source of truth for: current implementation and verification state

## 1. One-Sentence Status

Shipping HOT V2.2–V2.4 local closeout is complete on the retained NewsNow stack: V2.0 trust rules remain sealed, V2.1 is implemented, Mock remains the default, and the fixes cover composed calendar sources, verified Calendar provenance source activation, unknown Feed publication times, three weather windows, wave/swell directions, source-specific official warning contracts and final Mock isolation. The latest local batch additionally enforces Portcast source age, separates Open-Meteo forecast-valid time from local fetch time, and limits Shekou official HTML parsing to `/ywgg/` operational notices. The final isolation batch separates AIS watch configuration from observations, derives AIS status duration only from same-source observations, and filters incompatible historical Events out of the current Event/HOT view without deleting them.
...
This pass is locally verified and deliberately keeps the prior public probe as historical evidence. The requested new public re-probe of the corrected URLs was not run: the current Windows runtime safety gate rejected the external request. No result below is presented as a fresh live verification.

| Area | Current contract / last evidence | Status |
|---|---|---|
| Portcast | HCM mapping is now `https://www.portcast.io/port-congestion/ho-chi-minh`; source-age threshold is 14 days; missing source date is `source_update_time_unknown`; stale pages retain real Portcast values but cannot create Port Events/HOT. Prior 8/8 probe remains historical; Nansha and old-route HCM dates were already older than 14 days and will classify stale under the new gate. | locally verified; corrected 8-port re-probe pending |
| Open-Meteo | `updatedAt` is forecast/current valid time; `sourceUpdatedAt` is undefined unless the API supplies a reliable source/model-run time; `fetchedAt` is local request/normalization completion time; h24/h72/d7 windows and directions remain. Prior 8/8 HTTP/parser probe remains historical. | locally verified; corrected 8-port re-probe pending |
| Shekou Official Feed | Registry and `sourceUrl` now use `/ywgg/`; parser consumes `/ywgg/` article links only, excludes `/gsxw/`, navigation and footer noise, and never uses `fetchedAt` as publication time. | locally verified; corrected page re-probe pending |
| The Loadstar / Maritime Executive | Prior probe recorded Loadstar 10 parsed RSS items and Maritime Executive transport failure; no code change to either parser. | historical evidence; re-probe pending |
...
- 2026-08-18 Real Provider Live Path Fixes (completed locally; corrected public re-probe pending): Portcast now uses the HCM `/port-congestion/ho-chi-minh` path, enforces a 14-day source-age gate with explicit stale/date-unknown errors, preserves only real stale values and blocks stale Port Event/HOT creation; Open-Meteo separates forecast-valid `updatedAt`, optional reliable `sourceUpdatedAt` and local `fetchedAt`; Shekou official Feed now targets `/ywgg/` and excludes `/gsxw/`/navigation/footer noise. The local suite is 170/170. The prior 2026-08-17 public probe remains historical; the corrected external re-probe was not run after the runtime safety gate rejected the request.
- Blockers: Node 24.15.0 cannot load the bundled Node 22 native module, so Node 24 startup uses the documented in-memory fallback. Production Nitro subroutes reproduce the existing `#nitro/index` package-import runtime error; Vite development routes are healthy. Public corrected-provider runtime remains pending, and AISStream/Calendarific are `pending_credentials`. GitHub CLI account authentication remains invalid; Git HTTPS uses the repository-local OpenSSL backend because Windows Schannel failed with `SEC_E_NO_CREDENTIALS`.
- Verification: prior local gate `pnpm test --run` = 146/146; final AIS/Event batch `pnpm test --run` = 161/161; Calendar source activation batch `pnpm test --run` = 165/165; current Provider/live-path `pnpm test --run` = 170/170; current `pnpm typecheck`, targeted eslint, `pnpm build`, no-network provider/context/Store smoke, Python stdlib SQLite migration smoke, `git diff --check`, `git status` and manual Neat Freak inventory passed; full `pnpm lint` retains four pre-existing errors in `server/api/shipping/settings.post.ts`, `shared/updated-sources.ts`, and `src/routes/__root.tsx`; `shared/updated-sources.ts` itself is unchanged; native better-sqlite3 and corrected public re-probe remain pending.
