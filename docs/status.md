...
- Verification: targeted identity/P2A/P2B tests passed (18/18); full `pnpm test --run` passed (259/259); `pnpm typecheck` and `pnpm build` passed; service-backed native restart smoke covered pure provisional-name watch → same-name provider/IMO/MMSI promotion for `DONG FANG FU` and kept one canonical entity/watch; full lint retains four pre-existing unrelated errors. P2C Background Runtime Foundation remains pending; AIS Tracking Runtime remains pending and was not implemented.
