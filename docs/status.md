...
| Shipping HOT UI/routes | implemented / locally verified; live pending | `src/routes/**`, `src/components/shipping/**` | `/`, `/vessels`, `/ports`, `/voyages`, `/events`, `/feed`, `/calendar`, `/settings` and detail routes; UI uses the aurora glass/bento system, conditional Calendarific Attribution and weather window controls |
...
- Active work: V2.2/V2.3/V2.4 local closeout and release verification are complete; live external-provider verification remains pending; V2.5 AIS / Port Derived Intelligence remains not started.
