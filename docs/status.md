...
> Evidence scope: local code / configuration / Git metadata; V2.0 is sealed, V2.1 is implemented, V2.2/V2.3/V2.4 local closeout gates passed, V2.5 AIS / Port Derived Intelligence is implemented and locally verified, and accepted V3 P0 Persistence, P1A Port Directory Foundation, P1B Mock Isolation, corrected P2A Search Foundation, P2B Identity Seal and P2C Background Runtime Foundation are implemented and locally verified. P2A's mocked VesselAPI contract now uses the official `filter.*` query parameters and `{ vessels: [...] }` response shape; P2B now persists search-result watches through the existing user-owned `vessel_watchlist`, resolves canonical vessel identity before metadata/cache writes, rejects same-name strong-identity merges and multi-identity conflicts, and keeps AIS Tracking Runtime pending. P2C is now complete/sealed: migration v6 uses `(provider_id, capability)` identity, manual `runNow` resets timer cadence, skipped results persist the next schedule, and failed bootstrap/start leaves no zombie singleton. Production still registers no real business jobs. The example configuration defaults to Mock and keeps VesselAPI only in the commented Real Mode example; `live contract not verified` because no VesselAPI key is configured in the environment or FileSecretStore. The final V2.5 trust and reconnect lifecycle seals are local-only: a 60-second Shekou Area probe opened the socket and sent one small bounding-box subscription but received no PositionReport, so Area remains `connection_verified / coverage_pending`. AISStream Watched remains `connection_verified / pending_observation`; Calendarific is `verified_live` for five HTTP 200/parser-success country responses with partial coverage; official-alert verification remains `live_pending`. V3 fixed Node `24.15.0`, verified `better-sqlite3` ABI `137`, established the side-by-side `.data/shipping-hot-v3.sqlite3` database, schema migration runner through v6, ownership-separated watchlists, SecretStore contracts, runtime/usage/translation schemas, P1A `port_directory` baseline, P1B `source_type` lineage isolation and P2A `vessel_metadata`/search cache. AIS Tracking Runtime and later Provider business remain deferred. Full lint retains four pre-existing errors outside this batch.
> Source of truth for: current implementation and verification state

## 1. One-Sentence Status

Shipping HOT V2.2–V2.5 local closeout is complete on the retained NewsNow stack, and V3 P0 Persistence, P1A Port Directory Foundation, P1B Mock Isolation, corrected P2A Search Foundation, P2B Identity Seal and P2C Background Runtime Foundation are implemented on the same modular monolith: SQLite is the only Shipping HOT persistence truth, App/DB bootstrap is separate from Port Directory readiness, the eight-port directory baseline is ready, watchlists are user-owned, Provider upserts are field-scoped, Real Mode reads only `real/imported/derived` lineage, and unavailable persistence/provider capability cannot silently fall back to Mock. P2A provides server-side Vessel name/IMO/MMSI/callsign discovery with a 24-hour SQLite metadata/search cache, a static-only VesselAPI adapter boundary, official `filter.*` search parameters, official `vessels` response parsing and environment-first/FileSecretStore-fallback secret loading; P2B adds canonical identity resolution, strict provisional promotion, conflict-safe metadata updates, search-result watch/unwatch/list APIs and a minimal UI without starting AIS Tracking Runtime. P2C is complete/sealed with composite runtime identity, cadence-safe manual execution, skipped-result scheduling and failure-safe bootstrap; its production registry is empty. The example configuration uses a safe Mock default; live VesselAPI contract not verified. V2.5 keeps Watched AIS separate from Area AIS, stores only bounded aggregate metrics, uses reliable-source timestamp semantics, five-minute real buckets with gap/restart reset, stationary-count trend rules and warning-only Event/HOT signals for fresh usable watched-port samples. The real Area probe reached `connection_verified / coverage_pending` without an observation, so no `verified_live` claim is made.

`Mock isolation: complete` for V3 Real Mode: repository lineage filters, unavailable real-provider defaults, Schedule Mock removal and Event/HOT mixed-evidence rejection are verified. P2A Search Foundation is locally verified with Real Mode Mock cache isolation, including the case where a FileSecretStore VesselAPI key cannot override an explicit Mock provider; P2B Identity Seal is locally verified with canonical identity separation/promotion, conflict rollback and restart persistence; P2C Background Runtime Foundation is locally verified with singleton bootstrap, no-overlap execution, failure isolation, SQLite sync history/provider runtime updates, stop behavior and native restart persistence. The corrected official VesselAPI mock contract is verified and `live contract not verified`. AIS Tracking Runtime, AIS observation evidence, official-alert live verification and remaining P2 watch/tracking work remain pending/deferred.
...
| Shipping HOT V3 P2C Background Runtime Foundation | complete / sealed; real business jobs pending | `server/database/migrations/006-p2c-runtime-foundation.ts`, `server/runtime/background-runtime.ts`, `server/runtime/bootstrap.ts`, `server/runtime/registry.ts`, `server/database/runtime-jobs.ts`, `server/runtime/background-runtime.test.ts`, `server/api/shipping/runtime.get.ts`, `server/plugins/background-runtime.ts`, `scripts/p2c-runtime-sqlite-smoke.ts` | Migration v6 safely rebuilds `provider_runtime` with `(provider_id, capability)` identity and preserves rows. Runtime keeps per-job no-overlap/failure isolation, `runNow` cancels the old timer and restarts cadence from completion, persists skipped next-sync state, and clears timers/running state on failed start. Production Registry is intentionally empty: AIS Tracking Runtime, Feed Auto Sync, Calendar Auto Sync, Voyage and Translation remain pending. Existing `GET /api/shipping` request-triggered Provider path is explicitly legacy/deferred and receives no new sync behavior |
...
- `BackgroundRuntime` is a process-level singleton with idempotent bootstrap/start, timer scheduling, `runNow()`, no-overlap protection, failure isolation and shutdown cleanup. The default production Registry is empty; test-only FakeJobs are not registered in production.
...
- Verification: targeted/runtime/database tests passed (20/20); full `pnpm test --run` passed (270/270), `pnpm typecheck` passed, `pnpm build` passed, native P0 and P2C process-A-write → close → process-B-read smoke passed, `git diff --check` passed, and full lint retains only the four pre-existing unrelated errors.
- Explicit deferred Workstreams: AIS Tracking Runtime = pending; Feed Auto Sync = pending; Calendar Auto Sync = pending; Voyage = pending; Translation = pending. No real business Job is registered.
