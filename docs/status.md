...
| Shipping HOT V2.5 AIS / Port Derived Intelligence | implemented / locally verified; live pending | `shared/ais-area.ts`, `server/providers/aisstream-area.ts`, `server/database/shipping.ts`, `server/shipping-store.ts`, `shared/shipping-engine.ts`, `src/components/shipping/pages.tsx` | Explicit `SHIPPING_AIS_AREA_PROVIDER=off|aisstream` area session, configured heuristic watched-port boxes, PositionReport-only bounded observations, TTL/sample/trend metrics, separate `ais_port_metrics` aggregate persistence and warning-only Event/HOT path; no Portcast field mutation, raw track table or real area smoke; Mock remains default |
...
- AIS / Port Derived Intelligence: Watched AIS and area AIS use separate Provider/session boundaries. `SHIPPING_AIS_AREA_PROVIDER=off` is the default rollback; `aisstream-area` subscribes only to small configured heuristic boxes for watched current-port identities and sends `FilterMessageTypes=["PositionReport"]` without `FiltersShipMMSI`. The system retains latest-per-MMSI observations in memory, prunes them at 15 minutes, persists only one bounded aggregate per port, requires five distinct MMSIs and three consecutive rising windows for a warning-only `ais_port_congestion_trend`, and excludes insufficient/stale/incompatible metrics from current Event/HOT. No external area observation was run in this task.
...
- Active work: V2.2/V2.3/V2.4/V2.5 local closeout and release verification are complete; the 2026-08-19 Calendar sync persisted-baseline seal closes the restart-state boundary; the 2026-08-18 Calendarific final operational-semantics seal closes unsupported-type trust and legacy local migration; the 2026-08-19 V2.5 Phase 1 implementation adds the independent AIS area session, bounded aggregate persistence and warning-only Event/HOT boundary; remaining live gates are Watched/area AIS observation evidence and official alerts; the 2026-08-15 frontend console-layout round (plan A) is complete and locally verified.
