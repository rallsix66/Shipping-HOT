...
- Legacy unscoped Calendarific facts are retired from the current normalized set only when the same country/date/name/type is represented by an incoming scoped subdivision/unknown fact. The old CalendarEvent and any ShippingEvent that referenced its old ID remain historical records, are excluded from the current operational view, and are not marked resolved. Legacy unscoped national, Official and Manual facts remain unaffected.
