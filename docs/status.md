...
> Evidence scope: local code, controlled Real Mode runtime, production HTTP smoke, configuration and Git metadata. V3 P0 Persistence through P3B Voyage / ETA Foundation and P3 Feed Freshness Batch 1 are implemented and locally verified. The controlled Real Data Activation smoke now exercises the approved existing AISStream, public Feed, Calendarific, Portcast public-page and Open-Meteo adapters through Background Runtime → SQLite → Repository → API and reports an actual SQL/Repository zero-Mock scan; the safe default remains Mock and `.env.local` is not changed. Feed source jobs, Calendar Runtime, Port and Weather Runtime jobs are registered; real Voyage remains blocked because no adapter/key capability is available, and no new Provider, secret or deployment was added. The approved review repair adds only migration `011-provider-usage-records`. V3 uses fixed Node `24.15.0`, ABI `137`, actual `better-sqlite3` `12.6.2`, schema v11 and `.data/shipping-hot-v3.sqlite3`.
> Source of truth for: current implementation and verification state

> Current V3 state: P3 Feed Freshness Batch 1 and the approved Real Data Activation runtime slice are implemented and locally verified. Version 10 reclassifies v9 Feed rows using strict absent/invalid/future/expired date handling; Feed current/history APIs filter before the 500-row cap; source-level Feed jobs preserve last-known data and isolate failures; Calendar, Port and Weather jobs persist through SQLite. The activation review repair adds a real zero-Mock SQL/Repository gate, shared Provider identity contracts, migration 011 `records_count`, root Provider failure codes, and per-source Feed Readiness aggregation. Readiness is profile-aware: Development Safe can be ready with Mock/Off, while Real Operational is blocked until every approved Job is real/enabled and every capability’s credentials/coverage are actually verified. Batch 2 follow-up review remains pending; do not expand into later business work or add providers/credentials without separate approval.

## 1. One-Sentence Status

Shipping HOT V2.2–V2.5 local closeout is complete on the retained NewsNow stack, and V3 P0 Persistence through P3B Voyage / ETA Foundation plus P3 Feed Freshness Batch 1 are implemented on the same modular monolith. SQLite is the only Shipping HOT persistence truth; Real Mode reads only `real/imported/derived` lineage and never silently falls back to Mock. The controlled activation path adds source-isolated Feed, Calendar, Port and Weather Runtime jobs while keeping AISStream watchlist tracking bounded and the Voyage Job disabled when only Mock is available. The isolated real smoke produced an observed zero-Mock scan, and the process-scoped Mock/Off production HTTP smoke read repository/API surfaces; no AIS PositionReport was available because the current watchlist has no eligible MMSI, and no real Voyage adapter is claimed.

`Mock isolation: complete` for V3 Real Mode: repository lineage filters, unavailable real-provider defaults, Schedule Mock removal and Event/HOT mixed-evidence rejection are verified. The activation runtime is locally verified for source-isolated Feed jobs, Calendarific persistence, eight Portcast-derived port rows, seven Open-Meteo weather risk rows, provider usage accounting and repository-only legacy `/api/shipping`; the smoke reports `actualMockRows` from the eight operational tables and fails when its total is nonzero. Feed API current/history, Calendar API, Runtime API and readiness API were checked in a built Nitro process. Live AIS observation coverage, VesselAPI search credentials, real Voyage adapter/ETA coverage and official weather-alert coverage remain pending; these are status boundaries, not claims of failure or permission to start the next phase.

V3 Readiness is profile-aware: it checks observed Node `24.15.0` / ABI `137`, actual CLI pnpm `10.30.3`, actual installed better-sqlite3 `12.6.2` plus native load, current schema/Port Directory and the exact approved Runtime Job set. Development Safe requires Mock/Off; Real Operational rejects Mock but reports credential, runtime and coverage state per capability. HTTP does not infer pnpm from an unreliable request; an unavailable user-agent is `skipped`/unverified and strict `ready` remains false. Runtime readiness stores `Map<capability, RuntimeReadinessJob[]>`, so Feed retains source detail and aggregates all-healthy/partial/all-failed states without last-job overwrite. The current Real Operational HTTP report is correctly `blocked` because Voyage is disabled and no real Voyage provider is configured.

## Current V3 Real Data Activation — 2026-08-29

- Controlled process-only `SHIPPING_DATA_MODE=real` smoke used existing server-side adapters and wrote to `.data/shipping-hot-v3.sqlite3`; it registered `ais-tracking`, `voyage-sync`, two source-level Feed jobs, `calendar-sync`, `port-sync` and `weather-sync`. AISStream completed with zero records because no eligible watched MMSI exists; Voyage remained disabled because no real adapter/key is configured. No Mock row was written.
...
- The Real activation smoke now scans `vessels`, `ports`, `voyages`, `feed_items`, `events`, `calendar_events`, `ais_positions` and `ais_latest_positions` through the actual native SQLite database and Repository lineage rules. It emits `actualMockRows` by table and asserts `total === 0`; the clean run and injected-Mock regression test cover both pass and fail paths.
...
- Last verified surface: 2026-08-29 V3 Real Data Activation review repair — full suite 348/348, typecheck, lint, production build, five native SQLite/runtime smokes, process-scoped Mock/Off Readiness smoke, isolated controlled Real activation smoke, production Nitro HTTP smoke and `git diff --check` passed. The activation persisted 8 Ports, 17 Feed items and 259 Calendar events with `actualMockRows.total=0`; Real Operational Readiness correctly remains blocked by the disabled Mock Voyage Job and missing real Voyage capability. Neat Freak Closeout is complete through the loaded skill and Windows-equivalent manual audit; the official Bash inventory sub-step is `pending/unavailable` because Bash and `scripts/audit-inventory.sh` are absent.
...
| Shipping HOT V3 P2A Search Foundation | implemented / locally verified; live contract not verified; remaining P2 watch/tracking deferred | `server/database/migrations/005-p2a-search-foundation.ts`, `shared/vessel-search.ts`, `server/database/vessel-search.ts`, `server/providers/vessel-search.ts`, `server/search/vessel.ts`, `server/search/port.ts`, `server/api/shipping/search/**`, `example.env.server` | `vessel_metadata` stores static discovery identity and `vessel_search_cache` stores normalized 24-hour results; VesselAPI uses official `filter.name/imo/mmsi/callsign`, `{ vessels: [...] }`, and `name/imo/mmsi/call_sign/vessel_type/country` mapping with legacy fallback; `FileSecretStore` keeps environment priority over `.data/provider-secrets.json`; `example.env.server` defaults search to Mock and comments the Real Mode VesselAPI configuration; no AIS WebSocket, Tracking Runtime, Feed, Calendar, Voyage, Translation or Watchlist workflow |
| Shipping HOT V3 P2B Identity Seal | implemented / locally verified; consumed by P3A AIS Runtime | `server/search/vessel-watchlist.ts`, `server/search/vessel-watchlist.test.ts`, `server/database/vessel-search.ts`, `server/api/shipping/search/vessels/watch*.ts`, `src/components/shipping/pages.tsx`, `scripts/p0-native-sqlite-smoke.ts` | Search results use existing user-owned `vessel_watchlist`; strong identity is resolved by exact IMO, source+providerRecordId and MMSI before a strictly provisional source+normalized-name fallback that requires the existing row to have no IMO/MMSI/providerRecordId. Same-name strong identities never merge or cross-remove, conflicts return `identity_conflict` transactionally, canonical ids/cache/watch rows remain stable through promotion and MMSI changes, and persistence survives restart. No-MMSI entries remain unavailable for AIS Tracking; P3A sends only watched entries with existing MMSI and does not guess identifiers |
...
| Shipping HOT V3 P3A AIS Tracking Runtime Foundation | implemented / locally verified; live AIS observation pending | `server/database/migrations/007-p3a-ais-tracking.ts`, `server/database/ais-positions.ts`, `server/providers/ais/`, `server/runtime/ais-tracking-job.ts`, `server/runtime/registry.ts`, `server/api/shipping/vessels/[id]/position.get.ts`, `src/components/shipping/data.ts`, `src/components/shipping/pages.tsx`, `scripts/p3a-ais-sqlite-smoke.ts` | `ais_positions` preserves history and `ais_latest_positions` serves fast latest reads. AIS Runtime reads only `vessel_watchlist` entries with `ais_enabled=true` and MMSI, rejects unknown/invalid positions, keeps last-known data on Provider failure, isolates Mock positions from Real Mode, persists `sync_runs`/`provider_runtime`, and exposes only a minimal latest-position API/UI. AISStream adapter live contract 修复完成：bounded PositionReport reads include global BoundingBoxes, batch watchlist MMSI values at 50 per request, reject protocol errors with safe codes, and count only actually inserted history rows; no long-lived WebSocket service, map, trajectory, Feed, Calendar or Translation work was added |
...
| Shipping HOT V3 Readiness Gate | implemented / locally verified; Real Operational intentionally blocked | `server/services/v3-readiness.ts`, `server/api/shipping/readiness.get.ts`, `scripts/v3-readiness.ts`, `server/services/v3-readiness.test.ts` | Two profiles; actual Node/ABI, installed better-sqlite3/native load and CLI pnpm checks; HTTP pnpm absence is skipped/unverified and cannot yield `ready=true`; exact profile-specific Runtime Job set, capability/runtime/credential/freshness state, Feed source aggregation and Mock isolation. Current Real report blocks on disabled Mock Voyage Job and missing Voyage adapter/key |
...
- `AisTrackingProvider` is a server-only contract with `subscribe`, `unsubscribe` and bounded `getLatestPositions`. The AISStream adapter maps PositionReport MMSI, latitude, longitude, speed, course, heading, navigation status and source timestamp through a short-lived timeout-bounded read; it does not create a long-lived WebSocket service.
...
Next: review this V3 Real Data Activation review-gap closeout. The approved activation slice is complete locally, but Real Operational Readiness remains blocked by the disabled Mock Voyage Job, missing VesselAPI/Voyage capability evidence, missing AIS PositionReport observation and pending official weather-alert coverage. Do not enter Translation, a new Provider, real Voyage/Schedule or live AIS expansion without separate user confirmation.
...
| Runtime | changed-and-verified with explicit blockers | Node 24.15.0 / ABI 137 native read/write passed; isolated activation smoke produced the observed eight-table `actualMockRows.total=0`; process-scoped Mock/Off Nitro HTTP smoke passed all listed API surfaces; AIS produced no observation and Voyage is disabled without a real adapter/key | Keep Real Operational blocked until each capability is actually evidenced |
| Documentation | changed-and-verified with live caveat | Status, architecture, V3 plan, Provider Matrix, live verification and Voyage gap record schema v9/v10/v11, Feed current/history APIs, source-level Runtime jobs, actual zero-Mock evidence, usage semantics, toolchain/Readiness boundary and remaining provider limits | Review activation evidence; do not infer pending coverage from adapter code |
