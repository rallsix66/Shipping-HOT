...
> Evidence scope: local code / configuration / Git metadata; V2.0 is sealed, V2.1 is implemented, V2.2/V2.3/V2.4 local closeout gates passed, V2.5 AIS / Port Derived Intelligence is implemented and locally verified, and accepted V3 P0 Persistence, P1A Port Directory Foundation, P1B Mock Isolation, corrected P2A Search Foundation and P2B Watchlist Integration are implemented and locally verified. P2A's mocked VesselAPI contract now uses the official `filter.*` query parameters and `{ vessels: [...] }` response shape; P2B now persists search-result watches through the existing user-owned `vessel_watchlist`, resolves canonical vessel identity before metadata/cache writes, prevents identity conflicts from auto-merging, and keeps AIS Tracking Runtime pending. The example configuration defaults to Mock and keeps VesselAPI only in the commented Real Mode example; `live contract not verified` because no VesselAPI key is configured in the environment or FileSecretStore. The final V2.5 trust and reconnect lifecycle seals are local-only: a 60-second Shekou Area probe opened the socket and sent one small bounding-box subscription but received no PositionReport, so Area remains `connection_verified / coverage_pending`. AISStream Watched remains `connection_verified / pending_observation`; Calendarific is `verified_live` for five HTTP 200/parser-success country responses with partial coverage; official-alert verification remains `live_pending`. V3 fixed Node `24.15.0`, verified `better-sqlite3` ABI `137`, established the side-by-side `.data/shipping-hot-v3.sqlite3` database, schema migration runner, ownership-separated watchlists, SecretStore contracts, placeholder runtime/usage/translation schemas, P1A `port_directory` baseline, P1B `source_type` lineage isolation and P2A `vessel_metadata`/search cache. AIS Tracking Runtime and later Provider business remain deferred. Full lint retains four pre-existing errors outside this batch.
> Source of truth for: current implementation and verification state

## 1. One-Sentence Status

Shipping HOT V2.2–V2.5 local closeout is complete on the retained NewsNow stack, and V3 P0 Persistence, P1A Port Directory Foundation, P1B Mock Isolation, corrected P2A Search Foundation and P2B Watchlist Integration are implemented on the same modular monolith: SQLite is the only Shipping HOT persistence truth, App/DB bootstrap is separate from Port Directory readiness, the eight-port directory baseline is ready, watchlists are user-owned, Provider upserts are field-scoped, Real Mode reads only `real/imported/derived` lineage, and unavailable persistence/provider capability cannot silently fall back to Mock. P2A provides server-side Vessel name/IMO/MMSI/callsign discovery with a 24-hour SQLite metadata/search cache, a static-only VesselAPI adapter boundary, official `filter.*` search parameters, official `vessels` response parsing and environment-first/FileSecretStore-fallback secret loading; P2B adds canonical identity resolution, conflict-safe metadata promotion, search-result watch/unwatch/list APIs and a minimal UI without starting AIS Tracking Runtime. The example configuration uses a safe Mock default; live VesselAPI contract not verified. V2.5 keeps Watched AIS separate from Area AIS, stores only bounded aggregate metrics, uses reliable-source timestamp semantics, five-minute real buckets with gap/restart reset, stationary-count trend rules and warning-only Event/HOT signals for fresh usable watched-port samples. The real Area probe reached `connection_verified / coverage_pending` without an observation, so no `verified_live` claim is made.

`Mock isolation: complete` for V3 Real Mode: repository lineage filters, unavailable real-provider defaults, Schedule Mock removal and Event/HOT mixed-evidence rejection are verified. P2A Search Foundation is locally verified with Real Mode Mock cache isolation, including the case where a FileSecretStore VesselAPI key cannot override an explicit Mock provider; P2B Watchlist Integration is locally verified with search identity deduplication and restart persistence; the corrected official VesselAPI mock contract is verified and `live contract not verified`. AIS Tracking Runtime, AIS observation evidence, official-alert live verification and remaining P2 watch/tracking work remain pending/deferred.
...
- Last verified surface: source inspection, targeted identity/P2A/P2B tests (15/15), full tests (256/256), typecheck, production build, native SQLite read/write, migration-aware process A incomplete-identity → process B IMO/MMSI promotion smoke including search watch `DONG FANG FU` in schema v5, P2A VesselAPI static-only/cache tests, P2B identity deduplication/conflict/no-MMSI and Watchlist API/service paths, Port Search tests, corrected public re-probe, targeted Calendarific scope/count probe and persisted Calendar sync test passed; full lint ran and retains four pre-existing unrelated errors with no new P2A/P2B lint errors. `live contract not verified` because neither the environment nor `.data/provider-secrets.json` contains a VesselAPI key. Neat Freak Closeout is complete via the loaded skill and manual Windows-equivalent audit; the official Bash inventory sub-step is pending/unavailable because Bash is not installed in this Windows environment.
...
| Shipping HOT V3 P2B Watchlist Integration | implemented / locally verified; AIS Tracking Runtime pending | `server/search/vessel-watchlist.ts`, `server/search/vessel-watchlist.test.ts`, `server/database/vessel-search.ts`, `server/api/shipping/search/vessels/watch*.ts`, `src/components/shipping/pages.tsx`, `scripts/p0-native-sqlite-smoke.ts` | Search results use existing user-owned `vessel_watchlist`; canonical identity is resolved by exact IMO, source+providerRecordId, MMSI without conflict, then provisional source+normalized name; an established canonical id is retained through IMO/MMSI promotion, metadata/cache ids are canonical, conflicts return `identity_conflict`, and Watchlist matches source+providerRecordId without duplicates. No-MMSI entries remain unavailable for AIS Tracking, and persistence survives restart; no WebSocket, AisTrackingService or long-running AIS runtime |
...
### Implemented V3 P2B Watchlist Integration — 2026-08-21
...
- New metadata identity uses IMO first, then `source:providerRecordId`, then provisional MMSI/name. Existing metadata is resolved by exact IMO, source+providerRecordId, MMSI only when there is no conflicting identity, and provisional source+normalized name; its canonical id is retained when later IMO/MMSI values arrive. Conflicting IMO/providerRecordId/MMSI matches return `identity_conflict` and are not auto-merged. Search cache and Watchlist always use the resolved canonical id; Watchlist additionally matches source+providerRecordId. MMSI is retained as the AIS lookup target; no-MMSI records remain watchable with `aisTrackingAvailable=false` and no fabricated MMSI.
- Verification: targeted identity/P2A/P2B tests passed (15/15); full `pnpm test --run` passed (256/256); `pnpm typecheck` and `pnpm build` passed; native restart smoke covered incomplete provider identity → complete IMO/MMSI promotion for `DONG FANG FU` and kept one canonical watch; full lint retains four pre-existing unrelated errors. AIS Tracking Runtime remains pending and was not implemented.
