...
> Evidence scope: local code / configuration / Git metadata; V2.0 is sealed, V2.1 is implemented, V2.2/V2.3/V2.4 local closeout gates passed, V2.5 AIS / Port Derived Intelligence is implemented and locally verified, and accepted V3 P0 Persistence, P1A Port Directory Foundation, P1B Mock Isolation, corrected P2A Search Foundation, P2B Identity Seal, P2C Background Runtime Foundation and P3A AIS Tracking Runtime Foundation are implemented and locally verified. P2A's mocked VesselAPI contract now uses the official `filter.*` query parameters and `{ vessels: [...] }` response shape; P2B now persists search-result watches through the existing user-owned `vessel_watchlist`, resolves canonical vessel identity before metadata/cache writes, rejects same-name strong-identity merges and multi-identity conflicts, and preserves no-MMSI watches without guessing. P2C is complete/sealed: migration v6 uses `(provider_id, capability)` identity, manual `runNow` resets timer cadence, skipped results persist the next schedule, and failed bootstrap/start leaves no zombie singleton. P3A adds a bounded AISStream PositionReport read, append-only position history/latest view, watchlist-MMSI filtering, AIS Runtime Job persistence and minimum latest-position API/UI; it does not add a long-lived WebSocket service. Production registers only the AIS tracking Job; Feed, Calendar, Voyage and Translation remain pending. The example configuration defaults to Mock and keeps VesselAPI/AISStream only in the commented Real Mode examples; `live contract not verified` because no real provider key is configured in the environment or FileSecretStore. The final V2.5 trust and reconnect lifecycle seals are local-only: a 60-second Shekou Area probe opened the socket and sent one small bounding-box subscription but received no PositionReport, so Area remains `connection_verified / coverage_pending`. AISStream Watched remains `connection_verified / pending_observation`; Calendarific is `verified_live` for five HTTP 200/parser-success country responses with partial coverage; official-alert verification remains `live_pending`. V3 fixed Node `24.15.0`, verified `better-sqlite3` ABI `137`, established the side-by-side `.data/shipping-hot-v3.sqlite3` database, schema migration runner through v7, ownership-separated watchlists, SecretStore contracts, runtime/usage/translation schemas, P1A `port_directory` baseline, P1B `source_type` lineage isolation, P2A `vessel_metadata`/search cache and P3A `ais_positions`/`ais_latest_positions`. Full lint retains four pre-existing errors outside this batch.
> Source of truth for: current implementation and verification state

## 1. One-Sentence Status

Shipping HOT V2.2–V2.5 local closeout is complete on the retained NewsNow stack, and V3 P0 Persistence, P1A Port Directory Foundation, P1B Mock Isolation, corrected P2A Search Foundation, P2B Identity Seal, P2C Background Runtime Foundation and P3A AIS Tracking Runtime Foundation are implemented on the same modular monolith: SQLite is the only Shipping HOT persistence truth, App/DB bootstrap is separate from Port Directory readiness, the eight-port directory baseline is ready, watchlists are user-owned, Provider upserts are field-scoped, Real Mode reads only `real/imported/derived` lineage, and unavailable persistence/provider capability cannot silently fall back to Mock. P2A provides server-side Vessel name/IMO/MMSI/callsign discovery with a 24-hour SQLite metadata/search cache, a static-only VesselAPI adapter boundary and environment-first/FileSecretStore-fallback secret loading; P2B adds canonical identity resolution, strict provisional promotion, conflict-safe metadata updates, search-result watch/unwatch/list APIs and a minimal UI without guessing MMSI. P2C is complete/sealed with composite runtime identity, cadence-safe manual execution, skipped-result scheduling and failure-safe bootstrap. P3A adds the AIS tracking contract, bounded AISStream PositionReport reads, SQLite history/latest persistence, watchlist MMSI filtering, failure isolation and minimum latest-position API/UI; it does not add Feed, Calendar, Voyage or Translation behavior or a long-lived AIS WebSocket service. The production registry contains only the AIS tracking Job. The example configuration uses a safe Mock default; live real AIS contract remains key-gated. V2.5 keeps Watched AIS separate from Area AIS, stores only bounded aggregate metrics, uses reliable-source timestamp semantics, five-minute real buckets with gap/restart reset, stationary-count trend rules and warning-only Event/HOT signals for fresh usable watched-port samples. The real Area probe reached `connection_verified / coverage_pending` without an observation, so no `verified_live` claim is made.

`Mock isolation: complete` for V3 Real Mode: repository lineage filters, unavailable real-provider defaults, Schedule Mock removal and Event/HOT mixed-evidence rejection are verified. P2A Search Foundation is locally verified with Real Mode Mock cache isolation, including the case where a FileSecretStore VesselAPI key cannot override an explicit Mock provider; P2B Identity Seal is locally verified with canonical identity separation/promotion, conflict rollback and restart persistence; P2C Background Runtime Foundation is locally verified with singleton bootstrap, no-overlap execution, failure isolation, SQLite sync history/provider runtime updates, stop behavior and native restart persistence; P3A AIS Runtime is locally verified with provider mapping, unknown/invalid filtering, watchlist MMSI filtering, Mock isolation, sync history/provider runtime health, last-known preservation, duplicate execution protection and native restart persistence. The corrected official VesselAPI mock contract is verified and live real-provider contracts remain key-gated. Feed, Calendar, Voyage and Translation, plus long-lived AIS observation coverage, remain pending/deferred.
...
- Last verified surface: source inspection, targeted identity/P2A/P2B/P2C tests, full tests (270/270), typecheck, production build, Nitro startup/API smoke (`/api/shipping/runtime` returned `running=true` with zero production jobs), service-backed native SQLite process A pure provisional-name watch → process B same-name provider/IMO/MMSI promotion smoke including search watch `DONG FANG FU` in schema v6, P2C native process-A-write → close → process-B-read runtime smoke for `provider_runtime`/`sync_runs`, P2C migration rebuild, multi-capability independence, fake-timer cadence, skipped scheduling and bootstrap recovery tests, P2A VesselAPI static-only/cache tests, P2B same-name strong-identity separation, conflict rollback, no-MMSI and Watchlist API/service paths, Port Search tests, corrected public re-probe, targeted Calendarific scope/count probe and persisted Calendar sync test passed; full lint ran and retains four pre-existing unrelated errors with no new P2C lint errors. `live contract not verified` because neither the environment nor `.data/provider-secrets.json` contains a VesselAPI key. Neat Freak Closeout is complete via the loaded skill and manual Windows-equivalent audit; the official Bash inventory sub-step is pending/unavailable because Bash is not installed in this Windows environment.
...
- Explicit deferred Workstreams after P2C: AIS Tracking Runtime was the next approved phase; Feed Auto Sync, Calendar Auto Sync, Voyage and Translation remained pending.
...
- Verification: P3A targeted tests passed (current targeted suite includes provider mapping, unknown MMSI, invalid coordinates, Mock isolation, watchlist filter, runtime success/failure, last-known preservation, no duplicate execution and stale/latest reads); native P3A process-A-write → process-B-read smoke passed; typecheck and build passed; full lint has only the four pre-existing unrelated errors. Feed Auto Sync = pending; Calendar Auto Sync = pending; Voyage = pending; Translation = pending.
...
Next: keep Watched/Area AIS external observation and official-alert live verification pending; retain Calendarific's verified-live/partial status; AIS Tracking Runtime, remaining P2 watch/tracking and later Provider work remain deferred. V3 P0/P1A/P1B Mock Isolation/P2A Search Foundation/P2B Identity Seal/P2C Background Runtime Foundation are complete locally; stop here for review.

## 10. Knowledge Closeout Surface

| Fact surface | State | Evidence / limitation | Action |
|---|---|---|---|
| Code | changed-and-verified | Shipping HOT shared Domain, Provider orchestration, P2A search metadata/cache, P2B user-owned Vessel Watchlist Service/API/minimal UI, P2C Background Runtime singleton/Repository/bootstrap/status API, identity deduplication, no-MMSI marker and native restart persistence smoke are covered by passing checks; runtime-relative Mock fixture timestamps and Event Engine test determinism remain covered | Treat code/config as authority for current implementation |
| Runtime | changed-and-verified | Node 24.15.0 / ABI 137 native read/write passed; P2C process-A-write → close → process-B-read runtime smoke and Nitro startup/API smoke passed; live external verification remains separate | Keep native persistence pinned; retain AIS/official-alert live caveats |
...
| Workspace | changed-and-verified with retained cleanup candidates | Manual Neat Freak-equivalent inventory found the ignored `.data/db.sqlite3`, `.env.local`, `dist`, `.git-publish-temp/` metadata, and the retained `.git-publish-work/` worktree containing its copied `AGENTS.md` plus untracked publish traces; `prototypes/` and `screenshots/` remain user workspace content. No secret value was copied into docs or output. | Review the final `git status`; no cleanup performed without explicit confirmation |
...
- Closeout: pending official `audit-inventory.sh` execution because Bash is unavailable in this Windows environment; the required manual Neat Freak-equivalent audit completed with the retained local database/env/build and publish-worktree cleanup candidates recorded above, and no cleanup action.
