...
> Evidence scope: local code, controlled Real Mode runtime, production HTTP smoke, configuration and Git metadata. V3 P0 Persistence through P3B Voyage / ETA Foundation and P3 Feed Freshness Batch 1 are implemented and locally verified. The controlled Real Data Activation smoke now exercises the approved existing AISStream, public Feed, Calendarific, Portcast public-page and Open-Meteo adapters through Background Runtime → SQLite → Repository → API with `mockRows=0`; the safe default remains Mock and `.env.local` is not changed. Feed source jobs, Calendar Runtime, Port and Weather Runtime jobs are registered; real Voyage remains blocked because no adapter/key entitlement is available, and no new Provider, secret, migration or deployment was added. V3 uses fixed Node `24.15.0`, ABI `137`, actual `better-sqlite3` `12.6.2`, schema v10 and `.data/shipping-hot-v3.sqlite3`.
> Source of truth for: current implementation and verification state

> Current V3 state: P3 Feed Freshness Batch 1 and the approved Real Data Activation runtime slice are implemented and locally verified. Version 10 reclassifies v9 Feed rows using strict absent/invalid/future/expired date handling; Feed current/history APIs filter before the 500-row cap; source-level Feed jobs preserve last-known data and isolate failures; Calendar, Port and Weather jobs persist through SQLite. Readiness is profile-aware: Development Safe can be ready with Mock/Off, while Real Operational is blocked until every approved Job is real/enabled and every capability’s credentials/coverage are actually verified. Batch 2 follow-up review remains pending; do not expand into later business work or add providers/credentials without separate approval.

## 1. One-Sentence Status

Shipping HOT V2.2–V2.5 local closeout is complete on the retained NewsNow stack, and V3 P0 Persistence through P3B Voyage / ETA Foundation plus P3 Feed Freshness Batch 1 are implemented on the same modular monolith. SQLite is the only Shipping HOT persistence truth; Real Mode reads only `real/imported/derived` lineage and never silently falls back to Mock. The controlled activation path adds source-isolated Feed, Calendar, Port and Weather Runtime jobs while keeping AISStream watchlist tracking bounded and the Voyage Job disabled when only Mock is available. The real smoke produced no Mock rows and the production HTTP smoke read the resulting repository data; no AIS PositionReport was available because the current watchlist has no eligible MMSI, and no real Voyage adapter is claimed.

`Mock isolation: complete` for V3 Real Mode: repository lineage filters, unavailable real-provider defaults, Schedule Mock removal and Event/HOT mixed-evidence rejection are verified. The activation runtime is locally verified for source-isolated Feed jobs, Calendarific persistence, eight Portcast-derived port rows, seven Open-Meteo weather risk rows, provider usage accounting and repository-only legacy `/api/shipping`; the smoke reported `mockRows=0`. Feed API current/history, Calendar API, Runtime API and readiness API were checked in a built Nitro process. Live AIS observation coverage, VesselAPI search credentials, real Voyage adapter/ETA entitlement and official weather-alert coverage remain pending; these are status boundaries, not claims of failure or permission to start the next phase.

V3 Readiness is profile-aware: it checks observed Node `24.15.0` / ABI `137`, actual CLI pnpm `10.30.3`, actual installed better-sqlite3 `12.6.2` plus native load, current schema/Port Directory and the exact approved Runtime Job set. Development Safe requires Mock/Off; Real Operational rejects Mock but reports credential, runtime and coverage state per capability. HTTP does not infer pnpm from an unreliable request; an unavailable user-agent is `skipped`/unverified and strict `ready` remains false. The current Real Operational HTTP report is correctly `blocked` because Voyage is disabled and no real Voyage provider is configured.
...
- Built Nitro HTTP smoke returned 200 for `/`, `/api/shipping/health`, `/api/shipping`, `/api/shipping/feed`, `/api/shipping/feed/history`, `/api/shipping/calendar` and `/api/shipping/runtime`. Legacy `/api/shipping` read persisted repositories only and reported no Mock rows. `/api/shipping/readiness` returned `REAL_OPERATIONAL`, `blocked`, with the exact blocking Voyage Job state.
...
- Last verified surface: 2026-08-29 V3 Real Data Activation — full suite 329/329, typecheck, lint, production build, five native SQLite/runtime smokes, Mock/Off Readiness smoke, controlled Real activation smoke and built Nitro HTTP smoke passed. The activation persisted 8 Ports, 17 Feed items and 259 Calendar events with `mockRows=0`; Real Operational Readiness correctly remains blocked by the disabled Mock Voyage Job and missing real Voyage capability. Neat Freak Closeout is complete via the loaded skill and Windows-equivalent manual audit; the official Bash inventory sub-step remains `pending/unavailable` because Bash and `scripts/audit-inventory.sh` are absent.
...
Next: review the controlled V3 Real Data Activation closeout. The activation runtime slice is complete locally, but Real Operational Readiness remains blocked by the disabled Mock Voyage Job, missing VesselAPI/Voyage capability evidence, missing AIS PositionReport observation and pending official weather-alert coverage. Do not enter Feed, Calendar, Translation, real Voyage/Schedule or live AIS expansion without separate user confirmation.

## 10. Knowledge Closeout Surface

| Fact surface | State | Evidence / limitation | Action |
|---|---|---|---|
| Code | changed-and-verified | Shipping HOT shared Domain, P2A/P2B search/watch, P2C Runtime, P3A AIS, P3B Voyage/ETA, P3 Feed current/history API and v9/v10 Feed migrations are covered by passing checks; activation adds source-level Feed, Calendar, Port and Weather Jobs, provider usage accounting and repo-only legacy reads | Treat code/config as authority for current implementation |
| Runtime | changed-and-verified with explicit blockers | Node 24.15.0 / ABI 137 native read/write passed; activation smoke and built Nitro root/health/shipping/feed/history/calendar/runtime/readiness HTTP smoke passed with `mockRows=0`; AIS produced no observation and Voyage is disabled without a real adapter/key | Keep Real Operational blocked until each capability is actually evidenced |
| Documentation | changed-and-verified with live caveat | Status, architecture, V3 plan, Provider Matrix, live verification and Voyage gap record schema v9/v10, Feed current/history APIs, source-level Runtime jobs, toolchain/Readiness boundary and remaining provider limits | Review activation evidence; do not infer pending coverage from adapter code |
