...
| Shipping HOT V3 P2C Background Runtime Foundation | implemented / locally verified; real business jobs pending | `server/runtime/background-runtime.ts`, `server/runtime/bootstrap.ts`, `server/runtime/registry.ts`, `server/database/runtime-jobs.ts`, `server/runtime/background-runtime.test.ts`, `server/api/shipping/runtime.get.ts`, `server/plugins/background-runtime.ts` | Process singleton with idempotent bootstrap/start, timer scheduling, per-job no-overlap guard, failure isolation, structured lifecycle logs, `sync_runs`/`provider_runtime` writes, restart cursor recovery from SQLite, `SHIPPING_RUNTIME_ENABLED` switch and shutdown hooks. Production Registry is intentionally empty: AIS Tracking Runtime, Feed Auto Sync, Calendar Auto Sync, Voyage, Translation and other real business jobs remain pending. Existing `GET /api/shipping` request-triggered Provider path is explicitly legacy/deferred and receives no new sync behavior |
...
### Implemented V3 P2C Background Runtime Foundation — 2026-08-24
...
- `RuntimeRepository` writes `sync_runs` (`running` → `success`/`failed`/`skipped`) and maintains `provider_runtime` health (`healthy`, `degraded`, `failed`, `disabled`, `never_succeeded`) with schedule cursor, timestamps, error fields and consecutive failures. The status endpoint is `GET /api/shipping/runtime` and returns only local non-sensitive state.
...
- Verification: targeted/runtime/database tests passed; full `pnpm test --run` passed (264/264), `pnpm typecheck` passed, `pnpm build` passed, native P2C process-A-write → close → process-B-read smoke passed, `git diff --check` passed, and full lint retains only the four pre-existing unrelated errors.
