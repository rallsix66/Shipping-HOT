...
- `vessel_watchlist` and `port_watchlist` own user state; Provider upserts update only Provider-owned columns with explicit conflict updates. `translation_cache`, `provider_usage`, `provider_runtime` and `sync_runs` schemas/contracts plus `ProviderConfig`/`ProviderSecret`/`SecretStore` contracts are present. AI adapters, VesselAPI, UN/LOCODE, AIS long connection, Feed/Calendar/Voyage/Translation adapters remain deferred.
...
- Restart-style V2 tests remain historical evidence for legacy Calendar reconciliation; the current P0 native restart smoke verifies process-A write → close → process-B read through SQLite. `mock-schedule` remains explicitly allowed only where the retained Schedule contract permits it.
...
- P0 implemented `schema_migrations`, `app_metadata`, independent `port_directory_status/version/imported_at`, ownership-separated `vessel_watchlist`/`port_watchlist`, explicit Provider-column conflict updates, `translation_cache`, `provider_usage`, `provider_runtime`, `sync_runs`, Provider contracts, server-only FileSecretStore and redacted ProviderRegistry metadata. P0 did not implement Port Directory import, VesselAPI, UN/LOCODE, AIS long connection, Feed, Calendar, Voyage or Translation adapters.
...
- Verification: `pnpm typecheck` passed; targeted P0 tests passed; full `pnpm test --run` passed `235/235`; `pnpm build` passed; `pnpm run smoke:p0-native` passed a real process-A write → close → process-B read; direct native SQLite read/write passed under Node 24 ABI 137. Full `pnpm lint` has exactly four pre-existing errors outside this batch.
