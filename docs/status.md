...
- 2026 targeted probe reconciliation: raw `201` − invalid date `0` − missing name `0` → `201` normalized candidates; scope-aware dedupe retained `201` unique facts (`MY=67`, including same-day different-subdivision facts), `48` local/unknown facts, and `201` merged provider facts. The old `198` direct / `189` operational counts are superseded measurements from the pre-scope key and must not be used as the current invariant. Repository JSON scope round-trip is covered by tests; native better-sqlite3 runtime remains pending.
...
- Regression coverage after the Calendarific scope/reconciliation closeout: 191/191.
