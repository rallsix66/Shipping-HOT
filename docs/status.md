...
> Evidence scope: local code / configuration / Git metadata; V2.0 is sealed, V2.1 is implemented, and V2.2/V2.3/V2.4 local closeout gates passed. The corrected 2026-08-18 public re-probe verified Portcast 8/8 page/parser responses (7 fresh, 1 stale), Open-Meteo 8/8 port responses and Shekou `/ywgg/` parsing; Loadstar returned 10 items and Maritime Executive remained failed. AISStream and Calendarific remain `pending_credentials`; official-alert verification remains `live_pending`. The 2026-08-18 trust/isolation closeout now prevents unscoped TMD/BMKG port association, expiry claims from incomplete RSS indexes and known failed Feed scheduling. Full lint retains four pre-existing errors and native SQLite runtime is pending on this Node 24 environment.
...
- JMA strict empty-result validation requires a JMA-specific structure (`#seawarning-container`, `#contents table` or `.jma-information-list`); a generic `<main>` no longer qualifies.
