...
- VesselAPI ETA is retained as an AIS/crew-reported ETA observation, not commercial Schedule. `destination_port` and an explicit Departure event port are resolved through the existing Port Directory; unknown origin, voyage number, ETD and inferred movement status remain optional/`unknown`. Source timestamps come only from the official ETA/Port Event timestamps.
...
- Readiness promotes Voyage to `verified_live/configured` only when the selected `vesselapi` credential, enabled Runtime Job, persisted real Repository evidence, valid destination and trusted source timestamp all exist. Missing credential is `credential_missing`; adapter readiness without evidence is `coverage_pending`. No new migration was added because migration 008 already permits the affected nullable fields.
