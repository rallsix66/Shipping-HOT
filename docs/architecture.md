...
> Architecture status: approved for local Mock implementation, V1 AISStream/Open-Meteo adapters, sealed V2.0 Data Trust Foundation, implemented V2.1–V2.5 local capabilities and the GFW Vessel Search addition; ADR-005 V3 Real-Data Boundaries is Accepted, P0 Persistence through P3B Voyage / ETA Foundation and P3 Feed Freshness Batch 1 are implemented/verified. The controlled Real Data Activation runtime slice is locally verified for AISStream, public Feed, Calendarific, Portcast public pages and Open-Meteo through Runtime → SQLite → Repository → API, with an actual zero-Mock SQL/Repository scan. GFW Vessel Search and canonical identity, AIS Vessel PositionReport tracking and AIS Area metrics now have isolated Real evidence; Real Voyage/ETA adapter coverage and official weather-alert coverage remain pending; Translation and later business phases remain deferred.
> Source of truth for: the current retained system structure and approved boundaries

> Current V3 status: Feed Freshness Batch 1 and source-isolated Feed Background Runtime are implemented/verified. Calendar, Port and Weather Runtime jobs are also registered in the controlled activation path. GFW Vessel Search is implemented behind the server-side Provider boundary with canonical identity history, provider-aware cache and Watchlist latest-MMSI targeting. AIS Vessel Tracking and the independent AIS Area Background Runtime have separate AISStream sessions and persistence boundaries; both have isolated live evidence. Readiness now has `DEVELOPMENT_SAFE` and `REAL_OPERATIONAL` profiles, validates the exact profile-specific Job set, keeps credential/coverage state separate from code readiness, and aligns Area `verified_live/configured` with qualifying persisted Real `ais_port_metrics` evidence plus the enabled Area Job. Real Operational is currently blocked by missing real Voyage adapter/key capability and pending official weather-alert coverage; ETA entitlement remains not verified.
