...
> Architecture status: approved for local Mock implementation, V1 AISStream/Open-Meteo adapters, sealed V2.0 Data Trust Foundation, implemented V2.1–V2.5 local capabilities and the GFW Vessel Search addition; ADR-005 V3 Real-Data Boundaries is Accepted, P0 Persistence through P3B Voyage / ETA Foundation and P3 Feed Freshness Batch 1 are implemented/verified. The controlled Real Data Activation runtime slice is locally verified for AISStream, public Feed, Calendarific, Portcast public pages and Open-Meteo through Runtime → SQLite → Repository → API, with an actual zero-Mock SQL/Repository scan. GFW Vessel Search and canonical identity live evidence are verified in an isolated Real probe; Real Voyage/ETA adapter coverage, AIS observation coverage and official weather-alert coverage remain pending; Translation and later business phases remain deferred.
> Source of truth for: the current retained system structure and approved boundaries

> Current V3 status: Feed Freshness Batch 1 and source-isolated Feed Background Runtime are implemented/verified. Calendar, Port and Weather Runtime jobs are also registered in the controlled activation path. GFW Vessel Search is implemented behind the server-side Provider boundary with canonical identity history, provider-aware cache and Watchlist latest-MMSI targeting. Readiness now has `DEVELOPMENT_SAFE` and `REAL_OPERATIONAL` profiles, validates the exact profile-specific Job set and keeps credential/coverage state separate from code readiness. Real Operational is currently blocked by missing real Voyage adapter/key capability, pending AIS observation and pending official weather-alert coverage; ETA entitlement remains not verified.
