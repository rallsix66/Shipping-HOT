...
- Batch 1 implements the Feed freshness boundary without registering a Feed Runtime Job: ordinary Feed items use a seven-day current window, operational/major items use fourteen days, official items honor explicit expiry within the cap, and unknown/malformed/future publication or effective times are quarantine/history-only.
- Migration 009 adds Feed current metadata and append-only `feed_item_history`, with an idempotent backfill for legacy `feed_items`; `ShippingRepository` is the SQLite boundary for current-window reads, history search, source-disappearance archiving and Mock/Real lineage filtering. Existing migrations 001–008 are unchanged.
...
| Project Architect/Neat Freak docs reconciliation | changed-and-verified | Docs, status and accepted ADR now record P3A AIS Tracking Runtime Foundation and P3B Voyage / ETA Foundation as implemented while Feed, Calendar and Translation remain deferred | Re-run closeout after any later implementation phase |
