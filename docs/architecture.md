...
- This seals zero-Mock gate coverage for the review only; it does not make the system `Real Operational Ready` while official weather-alert coverage remains pending and VesselAPI Voyage/ETA has no local credentialed live evidence. GFW Vessel Search live authentication/search evidence is separate from AIS observation and Voyage/ETA readiness.
...
- Readiness promotes Voyage to `verified_live/configured` only when the selected `vesselapi` credential, enabled Runtime Job, persisted real Repository evidence, canonical destination and trusted ETA source timestamp all exist. Missing credential is `credential_missing`; adapter readiness without evidence is `coverage_pending`. Mock Mode force-selects Mock Voyage and the direct factory rejects a VesselAPI selection fail-closed. No new migration was added because migration 008 already permits the affected nullable fields.
...
- Current verification state: V3 P0/P1A/P1B/P2A/P2B/P2C/P3A/P3B, P3 Feed Freshness, source-isolated Feed Runtime, Calendar/Port/Weather activation and the Readiness Review repair are recorded in `docs/status.md`; Readiness shares the Node/ABI/pnpm/better-sqlite3 contract between API and CLI and validates the exact profile-specific Job set. It includes typecheck, production build, native better-sqlite3 read/write, migration-aware persistence smokes, Real activation smoke and built Nitro HTTP smoke. `pnpm lint` is all green. Real Voyage adapter/ETA entitlement, live AIS observation and official-alert live criteria remain pending; Translation and later business phases remain deferred.
