...
> Architecture status: approved for local Mock implementation, V1 AISStream/Open-Meteo adapters, sealed V2.0 Data Trust Foundation, implemented V2.1–V2.5 local capabilities; ADR-005 V3 Real-Data Boundaries is Accepted, P0 Persistence, P1A Port Directory Foundation, P1B Mock Isolation, P2A Search Foundation, P2B Identity Seal, P2C Background Runtime Foundation, P3A AIS Tracking Runtime Foundation, P3B Voyage / ETA Foundation and P3 Feed Freshness Batch 1 are implemented/verified; P3 Feed Background Runtime Batch 2, Calendar and Translation remain deferred, real Voyage adapter coverage remains pending; V2.5 external area observation remains live pending
> Source of truth for: the current retained system structure and approved boundaries

> Current V3 status: Feed Freshness Batch 1 is implemented/verified; Feed Background Runtime Batch 2 remains pending. Batch 1 does not register a Feed Job or change the Readiness approved AIS/Voyage Job set.

## V3 Readiness Gate — 2026-08-25

- `server/services/v3-readiness.ts` is a local, redacted foundation gate. The Node `24.15.0` / ABI `137` / pnpm `10.30.3` / better-sqlite3 `12.6.2` values are declared expectations; a check is `pass` only when the corresponding runtime/package observation matches them. The gate also verifies current SQLite migration/schema, P1A Port Directory readiness, Mock-only provider configuration, and a running Background Runtime whose Job set exactly matches approved AIS Tracking + Voyage Sync.
- `GET /api/shipping/readiness` initializes the existing migration runner and reads local SQLite/runtime state plus the same toolchain contract used by `pnpm smoke:v3-readiness`. The CLI observes actual `pnpm --version` and actual installed better-sqlite3 package metadata/native load, then bootstraps/stops the Runtime around its check. HTTP observes pnpm only from its user-agent; if unavailable it returns `package-manager=skipped` and strict Readiness remains `ready=false`, never “verified”.
- The gate performs no external Provider request and marks live contract/coverage checks as skipped. It deliberately fails when a Real/paid Provider override is active, so Readiness cannot silently become Provider activation. Feed Freshness Batch 1 is implemented locally; Feed Background Runtime Batch 2, Calendar, Translation and real Voyage adapter work remain deferred.
...
- Current verification state: V3 P0/P1A/P1B/P2A/P2B/P2C/P3A/P3B, P3 Feed Freshness Batch 1 review fixes and the 2026-08-25 V3 Readiness Review repair are recorded in `docs/status.md`; Readiness now requires the initialized/running exact AIS/Voyage Job set and shares the Node/ABI/pnpm/better-sqlite3 contract between API and CLI. It includes typecheck, production build, native better-sqlite3 read/write and migration-aware process-A-write → close → process-B-read persistence smoke for runtime, AIS position, Voyage/ETA and Feed current/history stores. `pnpm lint` is all green. Feed Background Runtime Batch 2, live AIS observation, real Voyage adapter coverage and official-alert live criteria remain pending; Calendar and Translation remain deferred.
...
| Project Architect/Neat Freak docs reconciliation | changed-and-verified | Docs, status and accepted ADR now record P3A AIS Tracking Runtime Foundation, P3B Voyage / ETA Foundation and P3 Feed Freshness Batch 1 (schema v9/v10 and current/history API) as implemented; Feed Background Runtime Batch 2, Calendar and Translation remain deferred | Re-run closeout after any later implementation phase |
...
The current local suite is 321/321 with production build, typecheck, the four prior native restart smokes plus the P3 Feed current/history restart smoke and the P3B Voyage native persistence smoke, and an all-green `pnpm lint`; the earlier 296/296 count is historical and superseded by the 2026-08-25 Readiness Review, P3 Feed Freshness Batch 1 and P3B dual-model consistency gates. The 2026-08-24 closeout repair removed the four pre-existing lint errors and the follow-up repairs fixed accepted-record `sourceUpdatedAt` handling and normalized Voyage Snapshot/API reads.
