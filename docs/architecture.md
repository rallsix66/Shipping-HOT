...
- Current verification state: current Provider/live-path batch 173/173 tests, typecheck, targeted lint, build, corrected public Portcast/Open-Meteo/Shekou re-probe, no-network provider-mode/context/Store smoke, Python stdlib SQLite migration smoke, `git diff --check` and `shared/updated-sources.ts` stability passed. Full lint retains four pre-existing errors outside this batch; native better-sqlite3 runtime and AIS/Calendarific/official-alert credentials or live criteria remain pending. Model weather and official weather alerts are independently configured and freshness-tracked.
