...
> Architecture status: approved for local Mock implementation, V1 AISStream/Open-Meteo adapters, sealed V2.0 Data Trust Foundation, implemented V2.1–V2.5 local capabilities; ADR-005 V3 Real-Data Boundaries is Accepted, P0 Persistence through P3B Voyage / ETA Foundation and P3 Feed Freshness Batch 1 are implemented/verified. The controlled Real Data Activation runtime slice is locally verified for AISStream, public Feed, Calendarific, Portcast public pages and Open-Meteo through Runtime → SQLite → Repository → API, with an actual zero-Mock SQL/Repository scan. Real Voyage/ETA adapter coverage, VesselAPI credentials, AIS observation coverage and official weather-alert coverage remain pending; Translation and later business phases remain deferred.
> Source of truth for: the current retained system structure and approved boundaries

> Current V3 status: Feed Freshness Batch 1 and source-isolated Feed Background Runtime are implemented/verified. Calendar, Port and Weather Runtime jobs are also registered in the controlled activation path. Readiness now has `DEVELOPMENT_SAFE` and `REAL_OPERATIONAL` profiles, validates the exact profile-specific Job set and keeps credential/coverage state separate from code readiness. Runtime readiness preserves multiple jobs per capability and aggregates Feed source state. Real Operational is currently blocked by the disabled Mock Voyage Job and missing real Voyage adapter/key capability; ETA entitlement remains not verified.
...
- This seals zero-Mock gate coverage for the review only; it does not make the system `Real Operational Ready` while AIS PositionReport, real Voyage/ETA, VesselAPI credentials and official weather-alert coverage remain pending.
