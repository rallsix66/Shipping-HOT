...
- Current verification state: current Official Alert Trust + Feed Failure Isolation batch 178/178 tests, typecheck, targeted lint and `git diff --check` passed on top of the preceding Provider/live-path, corrected public Portcast/Open-Meteo/Shekou re-probe, no-network provider-mode/context/Store smoke, Python stdlib SQLite migration smoke and `shared/updated-sources.ts` stability evidence. Full lint retains four pre-existing errors outside this batch; native better-sqlite3 runtime and AIS/Calendarific/official-alert credentials or live criteria remain pending. Model weather and official weather alerts are independently configured and freshness-tracked.
