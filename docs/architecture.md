...
The current local suite is 354/354 with production build, typecheck, the prior native restart smokes, P3 Feed current/history persistence, P3B Voyage native persistence, the actual zero-Mock Real activation smoke, process-scoped Mock/Off Nitro HTTP smoke and an all-green `pnpm lint`; the earlier 296/296, 321/321, 327/327, 329/329 and 348/348 counts are historical and superseded by the activation review-gap closeout. The closeout repairs fixed accepted-record `sourceUpdatedAt` handling, normalized Voyage Snapshot/API reads, source-level Feed scheduling, repo-only legacy reads, Provider identity propagation, root failure codes and usage record counts.
