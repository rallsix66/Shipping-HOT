...
- VesselAPI ETA is retained as an AIS/crew-reported ETA observation, not commercial Schedule. Returned ETA/Port Event identities are checked against the requested vessel; `destination_port` and an explicit Departure event port are resolved through the existing Port Directory. Stable ETA episodes use the canonical local vessel ID plus a safe normalized official destination key. Unknown origin, voyage number, ETD and inferred movement status remain optional/`unknown`; ETA `timestamp` alone owns Voyage freshness, while Port Event timestamps are enrichment-only.
