...
- Current verification state: the Calendarific scope/reconciliation closeout is 191/191 tests, with typecheck, targeted lint, build, `git diff --check`, targeted five-country Calendarific scope/count probe, local Mock/AIS-no-key/Calendarific-no-key runtime smoke and `shared/updated-sources.ts` stability evidence. The preceding corrected public Portcast/Open-Meteo/Shekou re-probe and all-real API/page smoke remain documented as prior baseline evidence. Full lint retains four pre-existing errors outside this batch; native better-sqlite3 runtime and AIS/Calendarific/official-alert credentials or live criteria remain pending. Model weather and official weather alerts are independently configured and freshness-tracked.
