...
- Each AISStream subscription keeps the existing global BoundingBoxes plus `FiltersShipMMSI` for the current batch and `FilterMessageTypes=["PositionReport"]`. `SubscriptionConfirmation` updates only in-memory/socket confirmation state and does not create a position or live success. Valid decoded PositionReports reuse the shared parser/mapper/ProviderError taxonomy and are passed immediately to `AisPositionRepository.savePositions()`.
- All live position persistence goes through one Promise-chain writer; individual failures are caught and exposed as `ais_persistence_failed` without claiming Provider health, and shutdown waits for pending writes after stopping acceptance of new messages. Provider runtime becomes `healthy` only after persistence completes, `failed` before the first persisted observation and `degraded` after a later stream failure; reconnect confirmation alone never restores health. Live frames do not increment `provider_usage` or create `sync_runs`; those remain bounded-job semantics, while continuous evidence is represented by tracker status, `provider_runtime`, `ais_positions` and `ais_latest_positions`.
