...
> Current V3 status: Feed Freshness Batch 1 and source-isolated Feed Background Runtime are implemented/verified. Calendar, Port and Weather Runtime jobs are also registered in the controlled activation path. GFW Vessel Search is implemented behind the server-side Provider boundary with canonical identity history, provider-aware cache and Watchlist latest-MMSI targeting. AIS Vessel Tracking and the independent AIS Area Background Runtime have separate AISStream sessions and persistence boundaries; both have isolated live evidence, while Area remains intentionally separate from Vessel readiness. Readiness now has `DEVELOPMENT_SAFE` and `REAL_OPERATIONAL` profiles, validates the exact profile-specific Job set and keeps credential/coverage state separate from code readiness. Real Operational is currently blocked by missing real Voyage adapter/key capability and pending official weather-alert coverage; ETA entitlement remains not verified.
...
- AIS Area live evidence is `verified_live` for the dedicated Area gate, but the current Readiness model intentionally reports `provider=aisstream`, `runtime=healthy`, `freshness=fresh`, `liveVerification=coverage_pending`, `status=coverage_pending`; this is `readiness_alignment_pending` for a later scoped change. Real Voyage/ETA and official weather-alert work remain out of scope.
