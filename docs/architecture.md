...
- The smoke persisted 8 Port rows, 17 Feed rows and 259 Calendar rows with an actual schema-discovered `actualMockRows.total=0` scan; AIS tracking completed with zero records because there is no eligible watched MMSI. Built Nitro HTTP smoke returned 200 for root, health, legacy Shipping Snapshot, Feed current/history, Calendar and Runtime. Legacy `/api/shipping` is repository-only and did not invoke a Provider.
...
- `AisTrackingJob` is registered through `BackgroundRuntime` with capability `ais_tracking`. It reads only `vessel_watchlist` entries where `ais_enabled=true`, canonical metadata is real-mode eligible and the MMSI is valid; no-MMSI or malformed-MMSI watches remain available but are never guessed or sent to AIS.
...
The current local suite is 357/357 with production build, typecheck, P0/P2C/P3A native restart smokes, the isolated zero-Mock Real activation smoke and an all-green `pnpm lint`; the retained mixed local database separately contains pre-existing Mock/Test rows and is not a clean Real gate. The earlier 296/296, 321/321, 327/327, 329/329, 348/348, 354/354 and 356/356 counts are historical and superseded by this AIS verification batch. The closeout repairs fixed accepted-record `sourceUpdatedAt` handling, normalized Voyage Snapshot/API reads, source-level Feed scheduling, repo-only legacy reads, Provider identity propagation, root failure codes, usage record counts and AIS trusted-timestamp/target filtering.
