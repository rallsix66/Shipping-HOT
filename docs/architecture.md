...
- The Real activation smoke uses the actual native SQLite connection to scan all eight operational tables for Mock lineage (`source_type`, normalized JSON and provenance evidence). Its `actualMockRows` object is an observation, not a constant, and a nonzero total fails the process. The approved review repair uses schema v11 migration `011-provider-usage-records` for `records_count`; `provider_usage.request_count` remains a capability sync invocation count rather than a claimed HTTP request/quota count.
