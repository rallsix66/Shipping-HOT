...
- `RuntimeJob` is limited to `id`, `providerId`, `capability`, `intervalMs`, `enabled` and `run()`. `SyncResult` contains status, record counts, source update time and redacted error fields, plus an optional cache-only marker that tells Runtime to preserve existing Provider evidence. The production Registry is empty in P2C; AIS, Feed, Calendar, Voyage and Translation Jobs remain separately approved workstreams.
...
- Unknown/out-of-target MMSI, missing trusted Provider timestamp and invalid coordinates never create a Vessel or position row; local `fetchedAt` is not promoted to source time. With no eligible watched target, `AisTrackingJob` returns `skipped/no_eligible_ais_targets` without invoking the Provider, and first-run Runtime health remains `never_succeeded`. Provider failures preserve the last-known position. The AIS position read service combines position TTL freshness with Runtime `sourceStatus`/`errorCode`; the latest-position API is Repository/RuntimeRepository → SQLite only, and the UI is limited to status/latest position/source/stale metadata without a map or trajectory surface.
...
Calendar follows `Server Start → SQLite → UI → background check → sync`; current year and next year are independently tracked by country/year coverage and last success. The default TTL is about 7 days (roughly 50–60 calls/month for six countries × two years), while a year change forces a check and manual refresh remains available. If both years are fresh, the Calendar Job records a cache-only skipped run without advancing source time; partial or actual sync results retain their own success/failure semantics. Calendar freshness is not inferred from Feed age windows.
