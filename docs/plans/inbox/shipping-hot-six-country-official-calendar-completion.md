...
## 当前产品状态与提案边界 — 2026-09-09（优先于下方历史提案）

- 已提交的现役产品是固定文件、只读的东南亚年度参考月历：五国共 100 条（ID 25、TH 23、MY 15、PH 21、VN 16），业务代码基线为 `main@e34115870804c0ef0040a568968c9ecce81af786`。`/calendar` 仅挂载新版月历；旧运营入口已移除，旧组件、后台和历史数据未删除。现役状态与既有验证证据统一引用 `docs/status.md` 顶部 Annual Reference Calendar 小节。
- 泰国全国 SOC 主依据正文仍待核验，其他国家的地域、行业、原始文本等既有证据限制不变；五国资料未完成人工双人复核，也不代表港口停运或全年完整覆盖。FullCalendar 仅是参考，未安装依赖。
- 印度尼西亚单国样本、其余国家候选整理和泰国补充是已完成的形成过程，只作为历史记录与候选证据，不再是当前执行指令。中国不纳入本次年度参考月历数据集，但现有系统中的中国 Calendar 能力未删除。
- 下方 Level 2 自动同步、`calendar_source_events`、legacy baseline、writer fingerprint、选择性 Runtime 和 migration 内容仍是 `proposal / awaiting approval`，整体暂停；它不是当前固定年度参考月历的实施要求，也未因 100 条展示数据接入而获批。
...
当前已核实业务代码基线为 `main@e34115870804c0ef0040a568968c9ecce81af786`；年度参考月历及泰国 23 条提升均已提交。2026-09-09 的实时 `git ls-remote origin refs/heads/main` 返回同一 SHA。后续文档同步不预先声明提交 SHA；工作区继续保留 `pages.tsx` 的换行状态与未跟踪 `.tmp/`。
...
2026-09-09 的提案/资料整理阶段没有读取 Secret、保留数据库或 `.tmp/` 内容，没有调用 Calendarific/其他付费 API，也没有启动 Provider 或 Web；它不涉及港口目录、JMA、资讯源、商业船期、Translation 或性能配置。
