...
> 当前执行状态：P3 Feed Freshness Batch 1 与来源独立 Feed Background Runtime 已完成；Calendar、Port、Weather Runtime 已纳入同一受控激活链路；GFW Vessel Search、canonical identity normalization、provider-aware cache 和 Watchlist latest-MMSI target flow 已实现并通过隔离 Real live probe；Translation T1/T2/T3A Runtime Foundation 及 Review Repair、T3B Feed Read、T3C Feed UI、T3D executable acceptance runner 已实现。T3B 在 `/api/shipping/feed` 与 `/api/shipping` 共享 provider-free batch cache mapper，成功译文优先、历史成功回退、无成功时原文；T3C 默认中文显示并提供“查看原文”，pending/unavailable 不阻塞 Feed；T3D 固定输入 `TRANSLATION_TEST_SOURCE_TEXT`、固定 endpoint/model，Phase 1 diagnostic + Phase 2 current real Feed field，显式 settings/secret/budget/circuit/Real Mode gates，hard max 2，复用 T3A claim/finalize/restart，并由独立 browser finalizer 合并 UI evidence，但本轮 retained settings disabled/monthlyBudget=0，未执行 DeepSeek。真实 DeepSeek live verification 为 pending，当前 external DeepSeek calls=`0`。Real Mode activation smoke 与生产 HTTP smoke 已验证真实写入/读取，且 `actualMockRows` 来自自动发现所有带 `source_type` 的 Shipping HOT 业务表的 SQL/Repository 扫描并要求总数为零。当前停止在 Real Operational Readiness：Voyage Provider path `liveVerification=verified_live`，但 focus-port coverage 为 `coverage_pending`；连续 AIS PositionReport 与 TMD/BMKG 官方天气预警均为 `verified_live`，JMA 仍 pending；Translation remains optional and outside the Readiness hard gate。
...
## 13. Translation 中文化方案（T1/T2/T3A Runtime Foundation 与 Review Repair、T3B/T3C/T3D executable acceptance runner 已实现）
...
| 测试 | T2 covers identifier/literal protection, cache key/version, failure original fallback, usage/budget gates, DeepSeek error/timeout/contract matrix, secret redaction and provider-free reads; T3A adds migration/repository/runtime/registry/readiness lifecycle tests；T3B/T3C/T3D adds batch selection, Feed isolation, UI disclosure, two-phase hard-call-counter, lifecycle/failure/restart and browser evidence separation tests |
