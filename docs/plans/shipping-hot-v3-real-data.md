...
## 13. Translation 中文化方案（T1/T2/T3A Runtime Foundation 与 Review Repair、T3B/T3C/T3D engineering 已实现）
...
### 13.4 两阶段持久化与单一事实源（T2 Foundation + T3A Runtime + T3B/T3C/T3D read/acceptance engineering）

1. Normalizer 先保存原文事实，确保抓取成功不依赖翻译。
2. T2 仅允许 fixed-input test POST 验证 Provider Foundation；T3A 已实现 bounded `translation-sync` Runtime work foundation；T3B/T3C 已实现生产 Feed read/display，T3D 仅完成 bounded preflight/evidence engineering，真实 live acceptance 仍 pending。
...
| 测试 | T2 covers identifier/literal protection, cache key/version, failure original fallback, usage/budget gates, DeepSeek error/timeout/contract matrix, secret redaction and provider-free reads; T3A adds migration/repository/runtime/registry/readiness lifecycle tests；T3B/T3C/T3D adds batch selection, Feed isolation, UI disclosure and bounded acceptance gate tests |
| 验收 | T3A/T3B/T3C/T3D engineering is locally verified with no external calls in this task; retained settings are disabled/monthlyBudget `0`, so DeepSeek live verification is `pending`. Translation remains optional and outside Readiness; no Event/HOT/calendar or broader entity translation is enabled |
