...
| Job | `AisTrackingJob` 使用 `provider_id=aisstream|mock`、`capability=ais_tracking`，读取 `vessel_watchlist` 中 `ais_enabled=true` 且有有效 9 位 MMSI 的 canonical metadata；无 eligible target 时返回 `skipped/no_eligible_ais_targets` 且不调用 Provider；写 `sync_runs` 与 `provider_runtime`，不自动猜 MMSI、不通过船名搜索 AIS |
