...
- Area session 为 server-process singleton，单飞连接、订阅复用、同 socket subscription replacement、至少 1 秒 debounce、有限自动重连预算和 idle close。初始 socket 不占预算；默认 `[1000, 2000, 5000, 10000]` 允许最多四次自动 reconnect，预算耗尽后不再后台建 socket；成功连接重置预算，后续显式 provider request 开启新 retry cycle，`close()` 取消 pending reconnect 并重置 retry state。Area subscription 只使用当前 watched ports 的小型 observation bounding boxes、`FilterMessageTypes=["PositionReport"]`，不使用 `FiltersShipMMSI`。
...
## V2.5 Final Reconnect Lifecycle Seal — 2026-08-19

- `AisAreaSession` 的 initial socket 不计入 retry budget；默认 `[1000, 2000, 5000, 10000]` 只允许四次自动 reconnect。达到预算后不再创建后台 socket。
- 成功 reconnect 会将 retry budget reset；预算耗尽后，下一次显式 `getPortMetrics()` 会开启新的 retry cycle。`close()` 会取消 pending reconnect、清理 socket/config 并 reset retry state。
- Verification：当前 `pnpm test --run` 为 `232/232`；AIS Area targeted lint、typecheck、build 和 `git diff --check` 通过。Area live evidence 没有重跑，仍为 `connection_verified / coverage_pending`；本轮未开始 V2.6。
...
当前 `shipping-store.ts` 会并行刷新 Vessel、Port、Voyage、Shipping Feed、Weather，失败时只把同一 Provider 的 last-known 标记为 stale/failed；没有同源历史时返回空集合/暂无数据，SQLite 不可用时保留内存 fallback。V2.2 另通过 `calendar_events` 保存年度事件事实，并在 `settings.calendarSync` 保存国家/年份/source coverage；partial/unknown coverage 缺失事件会保留为 stale degraded/failed，fresh incoming 恢复为 healthy，只有 complete coverage 才删除缺失事实。V2.3 Shipping Feed 默认仍是 Mock；显式 public 模式逐 Source 解析 RSS/HTML，启用源各自受 10 秒 fetch/body/parser deadline 保护，deadline 到期调用 `AbortController.abort()`，失败只影响该 Source，旧资讯保留原发布时间并标记 stale，未知发布时间保存空 `published_at` 并按真实 `fetched_at` retention/prune。Direct FeedItem → HOT 还要求 fresh/healthy evidence。V2.4 Weather 默认仍是 Mock；显式 Open-Meteo 模式一次请求 7 天 current/hourly 海况，本地计算 24/72/168 小时窗口，30 分钟 TTL 复用结果，单港失败只影响该港并保留同源 last-known，没有同源历史时不填充 `mock-weather`。官方预警港口关联只接受 alert title/summary/area、canonical port name/UNLOCODE 和 verified alias，不能由 provider-wide default `relatedPortIds` 产生；缺少当前索引且没有可靠 expiry 时保留为 stale/degraded lifecycle-unknown，不创建新 Event/HOT；已知别名映射到 canonical focus port。当前 Mock fixture 仍是产品可用的基础数据，因此 V2.0 必须让页面明确显示 `sourceType=mock`，而不是只显示一个看起来正常的“healthy”。当事件来源变成 stale、degraded、failed、disabled 或 never_succeeded 时，不创建新的事实事件，也不把已有 active 事件误判为 resolved；只有来源重新 fresh 且条件消失时才可 resolve。
...
- 原始方案基线曾要求本轮不实现 V2 功能；该基线已被用户批准的连续批次覆盖。当前已完成 V2.0、V2.1、V2.2、V2.3、V2.4 的本地实现与验证，外部 live verification pending，后续仍必须按 Gate 顺序推进，未获批阶段不得提前实现。
...
- Calendar ID 对旧的 national/unscoped 事实保持稳定；source merge/reconcile key 现在包含 `country + date + name + type + scope`，所以同名同日不同 subdivision 不会被错误合并。对旧版 Calendarific unscoped local，只有同源 incoming subdivision/unknown fact 匹配 country/date/name/type 时才执行 legacy identity migration；旧 normalized CalendarEvent 通过 `removedIds` 从当前 `calendar_events` 集合移除，但关联 ShippingEvent 保留审计历史、退出 current operational view、绝不自动 resolve。Official/Manual unscoped facts 不受影响。
...
- 港口详情组合天气、公告和拥堵证据。
- 已实现：列表/详情携带 `public`/`no_public_data`、stale/failed 状态和 Portcast 公共页面链接；HCM 使用 `/port-congestion/ho-chi-minh`；14 天 source-age gate 区分 fresh、`source_stale`、`source_update_time_unknown`、failed/no-public；stale Portcast 值不再创建新的 Port Event/HOT；Mock 仍是默认 Provider。

### V2.2

- 已实现国家日历页：国家、年份、月份、类型、影响等级、验证状态筛选。
- 已实现首页未来 14 天国家日历提醒入口；提醒通过现有 Event Engine 进入 HOT，普通提醒按 14/7/3 天窗口去重，新增 `government_special` 在首次发现时使用独立 announced 生命周期。
- 已实现日历详情的来源冲突、官方/手工优先级、最后核验、freshness 和来源链接展示。
- 已实现条件 Calendarific Attribution：仅当当前 Provider 或展示数据实际使用 Calendarific 时显示并链接；Calendarific Key 仅出现在 server-side outbound request。
- Event cards and timeline now label `firstDetectedAt` as 首次发现, `lastDetectedAt` as 最近确认 when it changes, and `resolvedAt` as 解决于; the wording applies to every Event type.
...
- Verification：preceding Provider/live-path batch 为 173/173；preceding Official Alert Trust + Feed Failure Isolation batch 为 178/178；preceding Final Alert Lifecycle + Feed Timeout Closeout batch 为 181/181；current Final Trust Boundary Seal batch 为 184/184；build、typecheck and targeted lint passed；active source 每源 10 秒 timeout + AbortController cancellation、单源 stale fallback、direct Feed HOT fresh/healthy gate、Shekou `/ywgg/` parser excludes `/gsxw/` and nav/footer fixtures and carries dated notices through Event/HOT；corrected public probe returned 5 `/ywgg/` items and zero `/gsxw/` items, with publication times unknown。
...
- Status：implemented / locally verified; live pending。
- Implemented：扩展 `createOpenMeteoWeatherProvider` 的 current/hourly wave/swell/wind 请求、24/72/168 小时风险窗口和方向字段、30 分钟 TTL、逐港 failure/同源 last-known stale；没有同源历史时首次失败返回暂无模型天气；`updatedAt`/`sourceUpdatedAt`/`fetchedAt` 语义分离，`generationtime_ms` 不冒充 source time；新增 `WeatherDetail.windows`、model/official risk labels、JMA 海上警报 HTML、TMD 公共 RSS 列表、BMKG 官方 RSS source-specific adapters 和 warning expiry/parse-failure 保留逻辑。TMD/BMKG 港口关联仅接受 alert title/summary/area、canonical port name/UNLOCODE 和 verified alias，provider-wide default `relatedPortIds` 不再生效；RSS 缺少 expiry 时 warning disappearance 只能保留 stale/degraded，不自动 expired；JMA generic `<main>` 不再通过 strict empty 判定。三个官方来源均标记 `live_pending`；`public` 不会启用它们，`experimental` 才显式允许 pending adapter。
- Verification：preceding Provider/live-path batch 为 173/173；preceding Remaining Real Provider Verification 为 174/174；preceding Official Alert Trust + Feed Failure Isolation 为 178/178；preceding Final Alert Lifecycle + Feed Timeout Closeout 为 181/181；current Final Trust Boundary Seal batch 为 184/184；typecheck and targeted lint passed；Open-Meteo timestamp/window/direction/cache fixtures passed；corrected public Open-Meteo probe verified 16 requests for 8 ports and `sourceUpdatedAt` remained undefined；missing-index lifecycle, alias mapping, strict JMA empty-marker and no-provider-default-port fixtures passed；official warning runtime remains pending。
...
- Implemented `server/providers/aisstream-area.ts` as a separate Area session. It is explicit-off by default, uses watched current-port identities, sends `FilterMessageTypes=["PositionReport"]` without `FiltersShipMMSI`, reuses a single socket with single-flight setup, one-second subscription debounce, finite reconnect budget with explicit cycle reset, idle close and bounded latest-observation/metric history. It never changes Watched AIS behavior.
...
- Local test evidence covers session boundaries, bounding boxes, no Watched MMSI filters, malformed messages, TTL/overlap/aggregation, persistence, provenance, failure/last-known behavior, finite reconnect budget/cycle reset/close cancellation, Event/HOT thresholds and Mock isolation. No real external Area smoke was started.
