...
| S8 | 最终合并 CI、授权发布、实际运行验收 | S7、单独合并/发布授权 | 分别记录 merged / CI passed / deployed / live verified |
...
| A1-03 | 工具链 | Windows/CI 目标/Docker 构建运行中 Node=24.15.0、pnpm=10.30.3；原生模块实际加载成功 |
| A1-04 | 运行产物 | `pnpm build`、`pnpm start` 与 Docker 实际启动成功；无测试 API/runner；生产包含运行所需原生模块 |
| A1-05 | 数据路径 | 用唯一隔离路径写入记录并重启后可读；dev/start/容器/CLI 指向预期位置；保留库未打开/未改写 |
| A1-06 | 访问边界 | 宿主机监听回环；恶意 Host、跨站写、错误类型/超大请求被拒绝；正常本地功能通过；无密钥进入响应/日志/静态文件 |
| A1-07 | 数据不丢 | 新旧版本对隔离数据副本的关注/设置等保留字段核对一致；物理卷名未悄悄改成新空卷 |
| A1-08 | 回归与收尾 | G 全过；真实浏览器访问保留路由通过；文档/规则/运行行为一致；无未解决阻塞 |
...
| A7-03 | 平台与容器 | Windows 启动/核心流、Linux 构建、Docker/Alpine 原生 SQLite 及容器重启读回通过 |
...
| A8-03 | CI 内容真实 | 安装/构建/类型/lint/全量测试/隔离端到端/容器检查等必需 job 全部成功；skipped 和无 run 不算通过 |
| A8-04 | 失败不发布 | CI 失败立即停止发布；定位后本地修复并重验，不删测试或重跑碰运气 |
| A8-05 | 发布授权 | 有明确发布/切换批准；镜像/产物绑定 merge SHA 或 digest；备份、数据路径和回退条件已确认 |
| A8-06 | 实际运行 | 实际服务显示正确版本；来源/模式/页面/关注/设置/全文/日历正常；本机重启数据保留 |
| A8-07 | 结论分层 | 分别给出 merged/CI passed/deployed/live verified/knowledge closed 及未清理项，不写含糊“完成” |
...
环境（实际 Node/ABI/pnpm、锁文件安装、原生 SQLite 加载）、工程（构建生成声明、typecheck、lint、全量 Vitest、新增功能测试）、隔离端到端（构建的 Nitro + 隔离 SQLite + 真实浏览器，Provider 用受控替身、出站阻断）、Windows（环境加载/路径/原生数据库/核心流）、Docker（构建自身镜像、检查实际 Node/原生 SQLite、启动唯一隔离卷、API 读取与重启读回）、安全/产物（无真实秘密/测试路由/旧产品入口；产物绑定最终 SHA）、差异（对实际 before→merge SHA 做空白/变更检查）、证据（失败也保存脱敏日志/截图；不因忽略失败或 `continue-on-error` 变绿）。CI 不注入真实业务密钥、不调用付费模型/商业船期、不复用用户 `.data`。
...
| S8 | 最终 merge CI 与授权发布/实机验收 | 待执行 | 合并/发布仍需单独授权 |

最终汇报必须回答：实际改了什么；退役了哪些旧业务；哪些共享能力和数据被保留；每阶段测试和验收结果；失败与修复；覆盖缺口和经批准延期；最终分支、PR、merge SHA 与 CI 运行；是否发布、实际服务版本；数据备份恢复结果；文档维护情况；尚未执行的检查与未清理对象。合并前 `docs/status.md` 记录准确候选本地验收，并把“合并后 CI/发布结果”指向现有 PR；合并后把 merge SHA、CI run、实机验证补在该 PR，**不再自动提交“CI 已通过”的文档改动到 main 造成新 CI**。

总体完成条件：全部纳入且未获准延期的验收项通过、准确最终 SHA 的 CI 成功、批准的实际运行验收和知识收尾完成，才能称本轮已交付。未授权发布只能称“实施/本地验收完成，待合并或发布”；船期或真实来源受阻必须明确部分完成。
